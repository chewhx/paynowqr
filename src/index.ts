...
export default class PaynowQR {
	...
	private readonly qrstring: string;

	constructor(opts: QROptions) {
		this.uen = opts.uen.trim();
		this.amount = opts.amount;
		this.editable = opts.editable;
		this.expiry = opts.expiry.trim();
		this.refNumber = opts.refNumber.trim();
		this.company = opts.company.trim();
		this.qrstring = this.generate();
	}
	...
	private padLeft(s: string, n: number, str?: string) {
		...
		const p = [
			...
			{ id: '01', value: '12' }, // ID 01: Point of Initiation Method 11: static, 12: dynamic
			{
				...
				value: [
					...
					{ id: '02', value: String(this.uen) }, // PayNow UEN (Company Unique Entity Number)
					{ id: '03', value: String(!this.amount || this.editable ? 1 : 0) }, // 1 = Payment amount is editable, 0 = Not Editable
					{
						id: '04',
						value: String(
							this.expiry ||
			...
			{ id: '54', value: String(this.amount || 0) }, // ID 54: Transaction Amount
			{ id: '58', value: 'SG' }, // ID 58: 2-letter Country Code (SG)
			{ id: '59', value: String(this.company || 'COMPANY') }, // ID 59: Company Name
			...
			value: [
				{
					...
					id: '01',
					value: String(this.refNumber || ''), // ID 01: Bill Number
				},
			],
		};
